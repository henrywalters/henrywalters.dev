...
export class AddLocationToTracking1599883426946 implements MigrationInterface {
    name = 'AddLocationToTracking1599883426946'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query("ALTER TABLE `page_visit` ADD `country` varchar(255) NOT NULL");
        await queryRunner.query("ALTER TABLE `page_visit` ADD `state` varchar(255) NOT NULL");
        await queryRunner.query("ALTER TABLE `page_visit` ADD `city` varchar(255) NOT NULL");
        ...
        await queryRunner.query("ALTER TABLE `project` CHANGE `url` `url` varchar(255) NULL");
        await queryRunner.query("ALTER TABLE `project` CHANGE `sourceControlUrl` `sourceControlUrl` varchar(255) NULL");
        await queryRunner.query("ALTER TABLE `project` CHANGE `videoUrl` `videoUrl` varchar(255) NULL");
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query("ALTER TABLE `project` CHANGE `videoUrl` `videoUrl` varchar(255) NULL DEFAULT 'NULL'");
        await queryRunner.query("ALTER TABLE `project` CHANGE `sourceControlUrl` `sourceControlUrl` varchar(255) NULL DEFAULT 'NULL'");
        await queryRunner.query("ALTER TABLE `project` CHANGE `url` `url` varchar(255) NULL DEFAULT 'NULL'");
        await queryRunner.query("ALTER TABLE `project` ADD CONSTRAINT `FK_62aa5845962762e0cefe729dfd7` FOREIGN KEY (`thumbnailId`) REFERENCES `user_file`(`id`) ON DELETE NO ACTION ON UPDATE NO ACTION");
        ...
        await queryRunner.query("ALTER TABLE `page_visit` DROP COLUMN `city`");
        await queryRunner.query("ALTER TABLE `page_visit` DROP COLUMN `state`");
        await queryRunner.query("ALTER TABLE `page_visit` DROP COLUMN `country`");
    }
